...
```text
 ...
 ① 数 (横)  --->  HPA (Horizontal)    --->  Podの「台数」を増減 
                                            (トラフィック増に対応)
                                            
 ② サイズ (縦) ->  VPA (Vertical)      --->  Pod 1台あたりの「CPU/メモリ」
                                            ★【ここが今回の論点】
                                            
 ③ 土地 (底)  --->  Cluster Autoscaler --->  Node (VM) の「台数」
                                            (Podが載りきらなくなったら発動)
...
「需要が不明」という状況に対し、GoogleがVPAを推す理由は以下の2点です。

1. **「適正サイズ」の自動判定**:
* エンジニアが「たぶん1コア必要」と勘で決めるのではなく、VPAが実際の使用実績を見て**「0.5コアで十分ですよ（コスト削減）」**、あるいは**「2コアないと死にますよ（安定性向上）」**と教えてくれます。


2. **Recommendation Mode（提案モード）の存在**:
* 勝手にPodを再起動（Autoモード）せず、まずは**「提案（Suggestion）」**だけしてくれるので、本番環境でもリスクなく導入できます。
...
* **需要が不明 / 適切なサイズを知りたい** ➔ **VPA** (Vertical)。
* **トラフィックの増減に合わせて台数を変えたい** ➔ **HPA** (Horizontal)。
* **VPA と HPA の併用**:
* 基本は**NG**（お互いにサイズと数を変えようとして干渉するため）。
* ただし、**VPAを「Recommendation Mode（提案だけ）」**にするなら併用OK。これが今回のQ40の「ベストプラクティス」の正体です。



#### **⚠️ 毒（誤答）の見分け方**

* **「Cluster Autoscaler で提案を受ける」** ➔ Cluster AutoscalerはVMを増やすだけで、Podのサイズ（需要）については無頓着なので「毒」。
* **「HPA だけで需要を解決する」** ➔ HPAは「台数」を増やすだけなので、1台あたりのメモリ不足などは解決できず「毒」。

---

### 🗣️ 音読用：脳内同期スクリプト

> 「Pod の食欲（リソース需要）がわからない？
> ならば **VPA（縦のスケーラー）** に聞け。
> 数を増やすのが **HPA**、
> 胃袋の大きさを決めるのが **VPA**。
> **需要不明 ＝ VPA Recommendation**。
> 欲張らずに『提案』から始めるのが Google 流だ。」
