# 🧠 TIL: 2025-10-27 Terraform Layer 2 – GCP接続準備

## 🎯 Goal / 目的
TerraformからGCPへ安全に接続する準備を行う。  
**Layer 2 = Provider設定 + 認証成功** がゴール。  
（リソースはまだ作成しない）
...
### 🕐 0〜5分｜環境チェック
- [ ] Codespacesを起動  
- [ ] `/devops_notes/Terraform` に移動  
- [ ] `bash scripts/sandbox-reset.sh` でsandbox初期化  
- [ ] `sandbox/02_gcp_connect/` に移動  

### 🕐 5〜15分｜構成ファイルの作成
> Terraformお約束ファイル4つを配置
- [ ] `versions.tf`（TerraformとProvider指定）  
- [ ] `providers.tf`（google provider設定）  
- [ ] `variables.tf`（project_id, region）  
- [ ] `main.tf`（コメントだけ）

### 🕐 15〜25分｜GCP認証
- [ ] `gcloud auth login --update-adc --no-launch-browser`  
- [ ] 表示されたURLをブラウザで開き、コード入力  
- [ ] 認証成功を確認（「You are now logged in」）  
- [ ] `gcloud config set project <PROJECT_ID>`  
- [ ] `gcloud projects list | grep <PROJECT_ID>` で確認  
...
## 📘 今日の作業リスト
- [ ] TIL作成・保存  
- [ ] `sandbox/02_gcp_connect/` に雛形ファイル作成  
- [ ] GCP認証（CLIブラウザ連携）  
- [ ] Terraformコマンドでplan成功確認  
- [ ] `terraform_gcp_minimal_design/02_gcp_connect/README.md` に記録  

---

## 🚀 Next Step
- **Layer 3:** Cloud Run “Hello World” のapply/destroyテストへ。  
- **教材化準備:** 英語版READMEを追記してUdemy/Note用に整理予定。  

---

### 💡 Notes:SISU mode
- 今回は認証とprovider確認に集中。  
- JSON鍵不要でブラウザ認証のみ利用（ADC方式）。  
- 明日はCloud Runの最小構成を追加予定。
