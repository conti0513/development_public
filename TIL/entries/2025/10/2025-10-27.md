# 🧠 TIL: 2025-10-27 — Terraform Layer 2 – Preparing GCP Connection

## 🗓️ **Terraform Sandbox – Today’s Recap**

### ✅ Achievements

1. **Installed gcloud CLI** in GitHub Codespaces.

2. **Connected to GCP project** `terraform-sandbox-lab`.

3. **Enabled API:** `cloudresourcemanager.googleapis.com`.

4. **Authenticated with ADC** (`gcloud auth login --update-adc`).

   * No service account key used or stored.

5. **Created minimal Terraform files**:
   `versions.tf`, `providers.tf`, `main.tf`, `terraform.tfvars`.

6. **Executed full Terraform workflow:**
...
7. **Verified security setup**

   * No credentials committed to repo.
   * `sandbox/` excluded via `.gitignore`.
   * Terraform state only stores safe metadata.

---

## 🧭 **Tomorrow’s Plan (★ Next Layer)**

| Step                               | Description                                                |
| ---------------------------------- | ---------------------------------------------------------- |
| **★ Layer 2 — GCP Connect Design** | Move Terraform state to **GCS backend** (`backend "gcs"`). |
| **★ Enable APIs**                  | Use `google_project_service` to manage required APIs.      |
| **★ Apply first resource**         | Create a **GCS bucket** (`google_storage_bucket`).         |
| **★ Confirm remote state**         | Ensure `.tfstate` is stored in GCS (not local).            |

---

🪜 **Next Directory:**
`/workspaces/development_public/devops_notes/Terraform/sandbox/02_gcp_connect/`

---

## 🚀 Next Step

* Move to **Layer 3:** Deploy and destroy a “Hello World” Cloud Run service.
* Prepare **English README version** for Udemy / Note documentation.

---

### 💡 Notes: SISU Mode

* Focused on GCP auth and provider validation today.
* Used **ADC (browser login)** — no JSON key required.
* Tomorrow: start adding Cloud Run minimal configuration.
