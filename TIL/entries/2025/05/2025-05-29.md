...
* Implemented a **Cloud Run service using Go**

  * Containerized the app with Docker for portability
  * Built a mechanism to transfer data securely via **SFTP triggered by Cloud Storage events**
  * Overcame limitations faced in Python, such as `open_session()` channel timeouts
  * **Go proved robust for legacy protocol communication**, paving the way to sunset VM dependencies

#### 🛠️ **Micro-Business Ops**

* Repaired and cleaned four camera lenses in the evening

  * Used this hands-on process to test assumptions about ROI vs manual effort
  * Gained insight into where to draw the line between perfection and productivity

---

### 🧭 Reflections

* **Using the right tool** (in this case, Go over Python) made a previously blocked system finally work
* Cloud-native design reduces ops overhead—**structure enables sustainability**
* Manual operations (like lens repair) offer learning, but must be balanced against energy and business scale
* "Structure, not struggle" is key to scaling both code and commerce
---
