...
* Built a **Cloud Run service using Go**

  * Packaged the app into a Docker container for portability
  * Implemented secure **SFTP data transfer triggered by Cloud Storage events**
  * Resolved issues that blocked Python (`open_session()` timeout)
  * **Go enabled reliable communication with legacy protocols**, allowing us to phase out VM use

#### 🛠️ **Micro-Business Ops**

* Refurbished and cleaned 4 vintage camera lenses

  * Evaluated the balance between **manual effort and business return**
  * Gained clarity on acceptable quality vs. productivity

---

### 🧭 Reflections

* Choosing the right tool (Go > Python) unlocked a path forward
* Cloud-native patterns reduce ops burden—**structure brings sustainability**
* Manual tasks (like lens repair) offer value but must be scoped realistically
* Key principle reaffirmed: **Structure, not struggle**
