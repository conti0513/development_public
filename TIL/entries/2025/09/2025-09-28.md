...
## 📌 本日の学習
- **SC-900 試験対策**として、Identity & Access Management の基本概念を学習した。  
- 主要な用語と原則を整理：  
  - **Authentication (認証 / AuthN)**: あなたは誰か？（入り口の鍵）  
  - **Authorization (認可 / AuthZ)**: 何ができるか？（部屋の鍵）  
  - **Federation (フェデレーション)**: 一度のログインで複数システムを利用（信頼関係に基づく）  
  - **Identity as the Primary Security Perimeter**: ネットワーク境界から ID が新しい境界へ  
  - **IAM がセキュリティ戦略の基盤であること**  
  - **RBAC / 最小権限 / SSO / MFA / PIM / JIT / パスワードレス認証**

## 🎯 振り返り
- クラウド時代では、ID が新しいセキュリティの境界になっている。  
- IAM（Identity & Access Management）はオプションではなく、**セキュリティ戦略の基盤**となる。  
- フェデレーションにより、ユーザー体験が向上し、パスワード管理の負担も軽減できる。  

## ✅ 次のステップ
- SC-900 模擬試験（100問）を継続して解く  
- 今日整理した Identity 関連の概念を Anki にカード化し、記憶を強化する  
