...
## 📌 本日の学習
- **SC-900 試験対策**として Microsoft Purview を学習。  
- Purview の概要：データセキュリティ、コンプライアンス、ガバナンスの統合基盤。  
- 主要な機能の整理：  
  - **Communication Compliance** → コミュニケーション内のポリシー違反を検知・防止  
  - **Information Barriers** → 部署間のやり取りを制限  
  - **Information Protection** → 機密データを分類・ラベル付与・暗号化  
  - **Information Governance** → データのライフサイクル管理（保持・削除・アーカイブ）  

## 🎯 振り返り
- 「Barriers = people」「Protection = data」「Governance = lifecycle」と整理すると理解しやすい。  
- Purview は **内部統制＋データ保護の総合プラットフォーム**として位置づけられる。  

## ✅ 次のステップ
- SC-900 模試で Purview 分野の出題パターンを確認  
- Ankiカード化して定着を強化する  
---
