...
**概要：** 弱電の対応で、施工業者が読みやすいように平面図に必要な要素だけをプロットした。

---

### 🛠️ Low-Voltage Planning in Store Project

### ✅ What I Did

* Worked on low-voltage planning (NTT lines, POS, traffic cameras, etc.)
* Plotted device locations directly on the store floor plan
* Simplified documentation: added only essential requirements assuming vendors will pick up details from the rough sketch

### 🎯 Purpose & Approach

* Made sure installers could easily estimate quantities and positions from the floor plan
* Avoided cluttering the drawing — kept only concise notes and key specs
* Added brief explanations for device placement to support on-site decision-making

### 💡 Takeaways

* Even rough sketches are effective if essential elements are clearly noted
* A well-annotated diagram helps avoid miscommunication and reduces back-and-forth with vendors
* These small efforts lead to smoother operations, fewer corrections, and less stress for everyone involved
