...
## 🧠 概要（日本語）
前任者が一切ドキュメントを残さず、弱電関連の調整業務が属人的でブラックボックス化していた。
自分はすべての作業を見える化し、バックログ起票・資料整備・ヘルプ作業のインシデント管理を実施。

これにより、
属人リスクの排除（透明性の向上）
証跡の蓄積（監査耐性の強化）
インシデント管理の導入（再発防止・対応スピードの向上）

といった、情報システム部門におけるセキュリティ基盤の構築を日常業務レベルで実践した。
---
...
* Coordinated with low-voltage vendors (NTT, people-counting camera, POS)
* Created backlog tasks to document and manage all pending issues
* Collected related emails and documents as artifacts of work
* Started logging helpdesk support in a shared spreadsheet to track small incidents
  → This acts as a primitive **incident management system**

### Why this matters:

The previous admin left **no documentation**, making vendor coordination opaque and risky.
I ensured **transparency** by turning all actions into trackable tasks, leaving clear digital footprints for future handovers.

This reduces:

* **Knowledge siloing**
* **Operational risks**
* **Security blind spots due to undocumented changes**

### Reflection:

Even for small or routine work, **documenting the “what/when/why” is part of secure IT admin practice**.
Visibility = security.
