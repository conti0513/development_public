...
- **概要**: オンプレ〜クラウド環境において、監視・通知・相関分析基盤を構築・運用。SIEMやクラウド監視、運用チームとの連携に強み。  
- **経験**:  
  - オンプレ時代、保険代理店システムの監視チームに所属。JP1・Shell・バッチを駆使して監視基盤を構築。  
  - 実績を認められてサブリーダーに昇格し、監視の運用だけでなく基盤開発まで担当。  
  - 2021年、Webマーケティング会社にてクラウド監視を導入。CloudWatchによるリソース監視やオートスケーリングを実装。  
  - SlackやLINEと連携し、社内外のアラート通知基盤を構築。  
- **実務適用例**:  
  - GCP Cloud Logging と Azure Sentinel を組み合わせ、PoCレベルでログ相関・インシデント分析基盤を実装。  
  - SOC運用移行を見据えたSIEM PoCを設計し、インシデント検知・対応の効率化を支援。  
- **規模感**:  
  - オンプレ時代：数千ユーザー規模の基幹システム監視。  
  - クラウド以降：100〜500名規模のサービスインフラ監視、全国展開するWebサービス基盤のPoC。  
- **関連規格**: ISO/IEC 27001 A.12（運用管理）、CIS Control v8（監視・ログ分析）  
- **経験レベル**: 運用オペレーション → サブリーダー → 監視基盤の開発 → クラウド監視設計 → セキュリティPoC、と段階的に経験を積んでいる。  
- **エピソード**:  
  - オンプレ時代は監視チームの一員としてスタート。実績を積みサブリーダーに昇格し、監視基盤そのものの開発を担当。  
  - この経験がベースとなり、クラウド移行後はAWS/GCPの監視基盤設計やSIEM PoCを推進。運用現場から開発・設計、セキュリティ監視まで一貫して関与してきた。  
ーーー