# Project 09：個人事業における EC・物流運用の整理

## 概要

個人事業として、  
**仕入れ・集客・販売・配送までを一人で完結させる EC 運用**を行っていました。

限られた時間とリソースの中で事業を継続するため、  
手作業を極力減らし、**運用を仕組みとして整理すること**を重視しています。

エンジニアリングそのものを目的としたプロジェクトではなく、  
**事業を回すために必要な部分を技術で支えた取り組みの記録**です。

---

## 使用していたサービス・ツール

### 販売・集客

- eBay（海外販売）
- Shopify（自社 EC）
- Instagram（集客・導線）

### 運用・自動化

- Python / Shell Script  
  - 在庫管理  
  - 出品データ更新
- Google スプレッドシート / Apps Script  
  - 収支管理  
  - 配送状況の整理

---

## 運用フロー（全体像）

仕入れから販売、配送、振り返りまでを、  
**できるだけ人手に依存しない流れ**として整理しています。

```mermaid
graph TD
    subgraph Sourcing
        A["仕入先 / オークション"] --> B["在庫管理"]
    end
    
    subgraph Sales
        B --> C["Shopify / eBay"]
        D["Instagram投稿"] --> C
    end
    
    subgraph Shipping
        C --> E["配送ラベル作成"]
        E --> F["購入者"]
    end
    
    subgraph Review
        F --> G["収支・実績管理"]
        G --> A
    end
````

---

## 運用上の工夫

### 手作業を増やさない

* 在庫更新や出品情報の反映はスクリプトで対応
* 同じ操作を繰り返さなくて済むよう、処理をまとめて実行できる形に整理

### 再実行できる仕組み

* データ更新処理は何度実行しても整合が取れる前提で整理
* 途中で失敗しても、最初からやり直せる状態を想定して運用

### 認証情報の扱い

* API 連携において、認証情報を分散管理しない形を意識
* 手元での管理負荷を増やさないことを優先

---

## この経験から得たこと

* 技術だけでなく、
  **物流・在庫・顧客対応の不確実性**を前提に考えるようになった
* 現場で実際に回る形でなければ、仕組みは意味を持たないと実感
* 「作る」よりも「壊れずに回し続ける」ことの難しさと重要性を理解

個人事業として全工程を経験したことで、
システムを **現場視点で整理し、無理のない形に落とす感覚** が身につきました。

---

## 補足

* eBay ストア
  [https://www.ebay.com/usr/analogcamerajp](https://www.ebay.com/usr/analogcamerajp)
* Instagram
  [https://www.instagram.com/analogcamera.jp/](https://www.instagram.com/analogcamera.jp/)

※ 本内容は、個人の経験をもとに整理した記録であり、
　特定の案件・組織・環境を前提としたものではありません。
